...
app.use('/auth', authRouter)
	...
	.get('/api/list', (req, res) => {
		r.table('list').run(r.conn, (err1, cursor) => {
			if (err1) {
				res.send(500, { error: err1.message });
			} else {
				cursor.toArray((err2, result) => {
					if (err2) {
						res.send(500, { error: err2.message });
					} else {
						result = result.map((item) => {
							...
							if (item.status === 'render') {
								...
								item.html += '<br>';
								item.html += `<a class='btn btn-primary' href='/dec/${item.id}.wav' role='button' download='${item.name.replace(/'/g, '\'')}.wav'>Download</a>`;
							}
	...
	.get('/queue', (req, res) => {
		...
		} else if (!config.get('admins').includes(`${req.user.login}@${req.user.type}`)) {
			res.status(403).render('error.html', { user: req.user, status: 403, message: 'You\'re not allowed to be in these realms!' });
		} else {
			...
			r.table('csrf')
				...
				.run(r.conn, (err1) => {
					if (err1) {
						...
						r.table('queue')
							.run(r.conn, (err2, cursor) => {
								if (err2) {
									res.send(500, { error: err2.message });
								} else {
									cursor.toArray((err3, result) => {
										if (err3) {
											res.send(500, { error: err3.message });
										} else {
											result = result.map((item) => {
												item.html = `<button type='button' class='btn btn-primary' onclick='copyText(\`${item.dectalk.replace(/`/g, '\\`').replace(/'/g, '\\\'')}\`)'>Copy</button>`;
	...
	.get('/approve', (req, res) => {
		if (!req.user) {
			...
			res.status(403).render('error.html', { user: req.user, status: 403, message: 'You\'re not allowed to be in these realms!' });
		} else if (!req.query.csrf || typeof (req.query.csrf) !== 'string' || req.query.csrf.length < config.get('limits').csrf.min	|| req.query.csrf.length > config.get('limits').csrf.max) {
			res.status(400).render('error.html', { user: req.user, status: 400, message: 'Invalid CSRF token provided' });
		} else if (!req.query.id || typeof (req.query.id) !== 'string' || req.query.id.length < config.get('limits').author.min	|| req.query.id.length > config.get('limits').author.max) {
			res.status(400).render('error.html', { user: req.user, status: 400, message: 'The ID was invalid, or outside the allowed range.' });
		} else if (!req.query.accept || typeof (req.query.accept) !== 'string' || !(req.query.accept === 'true' || req.query.accept === 'false')) {
			res.status(400).render('error.html', { user: req.user, status: 400, message: 'The Accept/Decline was malformed or not inserted.' });
		} else {
			r.table('queue')
				.get(req.query.id)
	...
	.get('/edit', (req, res) => {
		...
		r.table('csrf')
			...
			.run(r.conn, (err, result) => {
				...
				if (!req.query.id) return res.status(200).render('edit.html', { user: req.user, csrf });
				...
				r.table('list')
					...
					.run(r.conn, (err, result) => {
						...
						res.render('edit.html', { user: req.user, form: result, csrf });
					});
			});
	})
	.post('/edit', (req, res) => {
	...
	.get('/add', (req, res) => {
		...
		r.table('csrf')
			...
			.run(r.conn, (err) => {
				...
				res.render('add.html', { user: req.user, csrf, form });
			});
	})
	.post('/add', (req, res) => {
		if (!req.user) return res.status(401).render('error.html', { user: req.user, status: 401, message: 'You have not logged in yet' });
		...
		r.table('list')
			.run(r.conn, (err, cursor) => {
				...
				cursor.toArray((err, result) => {
					...
					result.filter(elem => elem.status == 'render').forEach((elem, i) => {
						setTimeout(() => {
							// Make a temp file to store the file
							tmp.file((err, path, fd, clean) => {
								if (err) throw err;
